...
function getLibraryAudioLi(audioName) {
  ...
  const foundItems = items.filter(
    (item) => item.querySelector("input").value == audioName
  );
  if (foundItems.length > 0) return foundItems[0];
  return null;
}
...
function fixNewAudioName(newAudioName) {
  ...
  const thisNameExists = (name) =>
    allAudioNames.some((anAudioName) => anAudioName == name);
  if (thisNameExists(newAudioName) == false) {
...
function renameLibraryAudioFromLi(
  li,
  oldFileName,
  newFileNameFixedNoDuplicate
) {
  ...
  for (const savedSetData of NodeCB.getAllSavedSetsWithAudiosData()) {
    ...
    if (savedSetData.audioNames.includes(oldFileName)) {
      ...
      if (audioIndex == -1) continue; // Somehow it used to happen... this should never happen, but better be safe than sorry
      NodeCB.retargetAudioShortcutInSetFolder(
  ...
  for (const h2 of queryAll(".setListSfxs li h2")) {
    if (h2.innerText == oldFileName) h2.innerText = newFileName;
  }
...
function getFavoriteSetsFromUINullIfEmpty() {
  return queryAll("#favoritesListSets h2").map((h2) =>
    h2.classList.contains("empty") || h2.innerText == "[Empty]"
      ? null
      : h2.innerText
  );
}
function isThereAnEmptySlotInFavorites() {
  ...
  console.log(lis);
  const emptyExists = lis.some((li) =>
    li.querySelector("h2").classList.contains("empty")
  );
  return emptyExists;
}
function getAnyEmptyFavoriteSet() {
  if (isThereAnEmptySlotInFavorites() == false) return null;
  const ul = document.querySelector("#favoritesListSets");
  const lis = Array.from(ul.children);
  const emptyLis = lis.filter((li) =>
    li.querySelector("h2").classList.contains("empty")
  );
  return emptyLis[0];
}
function removeAnyEmptyFavoriteSet() {
  if (isThereAnEmptySlotInFavorites() == false) return false;
...
function renameSet(newSetName) {
  function fixNewSetName(newSetName) {
    ...
    const thisSetNameExists = (setName) =>
      setData.some((sd) => sd.setName == setName);
    if (thisSetNameExists(newSetName) == false) {
      // set name does not exist
      return newSetName;
    }
    let setCopyIndex = 1;
...
function deleteSetByLi(setLi) {
  ...
  const favoriteLis = queryAll("#favoritesListSets li").filter(
    (li) => li.querySelector("h2").innerText == setName
  );
...
function deleteFavoriteSetByLi(favoriteLi, options) {
  ...
  // Update star
  const allSetNames = queryAll("#favoritesListSets li h2").map(
    (h2) => h2.innerText
  );
  if (allSetNames.some((name) => name == setName) == false) {
    // Only remove star if there is NOT another favorite of the same set
...
function tryAddSoundFromLibraryToSet(audioName, savedSetName) {
  if (doesSavedSetHaveAudio(savedSetName, audioName)) return;
  if (doesSavedSetHaveEmptySlot(savedSetName) == false) return;
...
function getAudioIndexInSet(savedSetName, audioName) {
  const audioLis =
    getSavedSetWithNameDom(savedSetName).querySelectorAll(".setListSfxs li");
  for (let i = 0; i < audioLis.length; i++) {
    if (audioLis[i].querySelector("h2").innerText == audioName) return i;
  }
  return null;
}
...
function getSavedSetAudioDom(savedSetName, audioName) {
  ...
  const soundsInThisSet = Array.from(
    savedSetLi.querySelectorAll(".setListSfxs li")
  );
...
function getCurrentlyActiveSetLi() {
  ...
  if (savedSetsLis.length == 0) return null;
...
function getButtonsAudioNamesNullIfEmpty() {
  return queryAll(".buttonInfo h2").map((h2) =>
    h2.innerText == "[Empty]" ? null : h2.innerText
  );
}
...
function updateLibraryAndSavedSetsTitleNumber() {
  query("#numberOfItemsLibrary").innerText =
    query("#libraryItems").children.length;
